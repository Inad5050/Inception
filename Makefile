...
# docker-compose down: Detiene los contenedores de los servicios definidos y elimina los contenedores y las redes que creó docker-compose up.
# --volumes: modifica el comportamiento de down para que, además de las acciones anteriores, elimine los volúmenes nombrados que están declarados en la sección volumes del fichero docker-compose.yml y los volúmenes anónimos adjuntados a los contenedores.
# volumen anónimo: en el contexto de Docker Compose, es un volumen de datos que Docker crea y gestiona, pero al que no se le asigna un nombre específico y predecible por el usuario en el fichero docker-compose.yml.
clean:
	docker-compose -f $(DOCKER_COMPOSE_FILE) down --volumes
	@echo "$(COLOR_GREEN)------------ MESSAGE: DOCKER SERVICES DOWN ------------ $(COLOR_RESET)"

# docker system prune: Es el comando base para eliminar datos no utilizados de Docker, como contenedores detenidos, redes sin uso y cachés de construcción.
# --all (o -a): Extiende la acción de prune para que no solo elimine los recursos "colgantes" (dangling), sino todos los recursos que no estén siendo utilizados por al menos un contenedor. Esto incluye la eliminación de todas las imágenes que no estén asociadas a un contenedor existente, incluso si no están "colgantes". 
# --volumes: Incluye en la limpieza todos los volúmenes no utilizados. Por defecto, docker system prune no elimina volúmenes para prevenir la pérdida accidental de datos. Esta bandera anula esa protección y elimina cualquier volumen que no esté conectado a un contenedor en ejecución o detenido. 
